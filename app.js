    const VIDEOS = [];
    let AUTO_MARATHON_PLAYLIST = Object.freeze({
      id: 'auto-todos',
    ...
    function highestRarityForActorCharacter(actorName, characterName) {
      const entries = VIDEOS.filter((video) =>
        (video.actor_de_doblaje || 'Sin actor') === actorName &&
        (video.personaje || 'Sin personaje') === characterName
      );
      if (!entries.length) return 'Bloqueado';
      const labels = ['Común', 'Raro', 'Épico', 'Legendario'];
      const best = entries.reduce((max, video) => {
        const rank = rarityRank(rarezasPermitidas(video.rareza || 'Común'));
        return rank > max ? rank : max;
      }, -1);
      return best >= 0 ? labels[best] : 'Común';
    }
    ...
    function groupByUniverse() {
      const grouped = VIDEOS.reduce((acc, item) => {
        ...
        universes.filter(Boolean).forEach((rawName) => {
          ...
          if (character) {
            acc[key].characters.add(character);
            if (hasGreetingVideo(item)) acc[key].unlockedCharacters.add(character);
          }
        });
        return acc;
      }, {});
    ...
    async function loadFromFirebase() {
      ...
      try {
        ...
        if (Array.isArray(data.universeNodes)) {
          state.universeNodes = mergeUniverseNodes(state.universeNodes, data.universeNodes);
          localStorage.setItem(UNIVERSES_STORAGE_KEY, JSON.stringify(state.universeNodes));
        }
    ...
    function buildAutoMarathonPlaylist() {
      ...
      AUTO_MARATHON_PLAYLIST = Object.freeze({
        id: 'auto-todos',
        name: 'Auto • Todos los personajes',
        source: 'auto',
        videoIds: Object.freeze(sortedPlayableVideos.map((video) => video.id))
      });
      state.marathon.playlists = [AUTO_MARATHON_PLAYLIST];
      const playlistExists = state.marathon.playlists.some((playlist) => playlist.id === state.marathon.selectedPlaylistId);
      if (!playlistExists) {
        state.marathon.selectedPlaylistId = AUTO_MARATHON_PLAYLIST.id;
      }
    ...
    function renderUniverseView() {
      ...
      const playableCharactersCount = new Set(
        playableVideos.map((video) => String(video.personaje || '').trim()).filter(Boolean)
      ).size;
      const universeMap = groupByUniverse();
        ...
        <section class="hud-panel universe-header universe-view-animated" style="--progress-color:${progressVisual.color};">
          <div class="universe-hero">
            ...
            <div class="universe-meta">
              ...
              <div class="universe-stat-lines">
                <p>Personajes: ${playableCharactersCount}</p>
                <p>Videos: ${playableVideos.length}</p>
              </div>
          ...
          <div class="universe-controls">
            <button
              ...
              aria-label="${state.showEditUniverseForm ? 'Cerrar edición' : 'Editar universo'}"
            >
              ✏️
            </button>
            ...
            <button
              ...
              aria-expanded="${state.showAddForm && state.universeAddMode === 'video' ? 'true' : 'false'}"
            >
              ${state.showAddForm && state.universeAddMode === 'video' ? 'CERRAR' : 'AGREGAR VIDEO'}
            </button>
            <button
              id="btn-add-blocked-character"
              class="neon-btn neon-action"
              aria-expanded="${state.showAddForm && state.universeAddMode === 'character' ? 'true' : 'false'}"
            >
              ${state.showAddForm && state.universeAddMode === 'character' ? 'CERRAR' : 'AGREGAR PERSONAJE'}
            </button>
          </div>
